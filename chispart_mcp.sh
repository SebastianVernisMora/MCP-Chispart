...
cmd_agents() {
  ...
  MCP_MAILBOX_ROOT="$MAILBOX" MCP_AGENT_NAME=codex MCP_AGENT_ROLE=coordinator \
    node scripts/chispart-mcp/agents/adapter-template.mjs &
  ...
  if [[ -n "${BLACKBOX_API_KEY:-}" ]]; then
    MCP_MAILBOX_ROOT="$MAILBOX" MCP_AGENT_NAME=blackbox MCP_AGENT_ROLE=dev-support \
      node scripts/chispart-mcp/agents/blackbox.mjs &
  ...
  if [[ -n "${MISTRAL_API_KEY:-}" ]]; then
    MCP_MAILBOX_ROOT="$MAILBOX" MCP_AGENT_NAME=mistral MCP_AGENT_ROLE=dev-support \
      node scripts/chispart-mcp/agents/mistral.mjs &
...
cmd_task() {
  ...
  node scripts/chispart-mcp/orchestrator.mjs task "$title" --repo "${1:-Yega-API}" --roles coordinator,qa
}

cmd_pump() {
  node scripts/chispart-mcp/orchestrator.mjs pump
}
...
cmd_tasks() {
  ...
  case "$sub" in
    list)
      node scripts/chispart-mcp/orchestrator.mjs tasks list "$@" ;;
    show)
      node scripts/chispart-mcp/orchestrator.mjs tasks show "$@" ;;
    report)
      node scripts/chispart-mcp/orchestrator.mjs tasks report "$@" ;;
    plan)
      node scripts/chispart-mcp/orchestrator.mjs tasks plan "$@" ;;
    close)
      node scripts/chispart-mcp/orchestrator.mjs tasks close "$@" ;;
    *)
...
cmd_send() {
  ...
  case "$sub" in
    change)
      node scripts/chispart-mcp/orchestrator.mjs send change "$@" ;;
    *)
...
cmd_nl() {
  ...
  case "$sub" in
    task)
      node scripts/chispart-mcp/orchestrator.mjs nl task "$@" ;;
    *)
      echo "Usage: $0 nl task \"<pedido NL>\" [--repo <repo>] [--roles r1,r2] [--agents a1,a2]" ;;
  esac
}
...
cmd_chat() {
  ...
  case "$sub" in
    new)
      node scripts/chispart-mcp/chat.mjs new ;;
    attach)
      node scripts/chispart-mcp/chat.mjs attach "$@" ;;
    *)
...
case "${1:-}" in
  ...
  clean)
    shift || true
    bash scripts/chispart-mcp/clean-mailboxes.sh "$@" ;;
  *)
